...
/// flags dropped). Nullable acquire_time/next_pay use SQL epoch projections and FROM_UNIXTIME writes (House uses DateTimeOffset?).
/// Map.get/put->indexer, containsKey->ContainsKey; building.getType()->GetType_(); Persistable.PersistentState
...
public class HousesDAO
{
    ...
    private const string ADD_HOUSE_QUERY = "INSERT INTO houses (id, address, building_id, player_id, acquire_time, settings, next_pay, sign_notice) "
        + " VALUES (?,?,?,?,FROM_UNIXTIME(? / 1000.0),?,FROM_UNIXTIME(? / 1000.0),?)";

    private const string UPDATE_HOUSE_QUERY = "UPDATE houses SET building_id=?, player_id=?, acquire_time=FROM_UNIXTIME(? / 1000.0), settings=?, next_pay=FROM_UNIXTIME(? / 1000.0), sign_notice=? WHERE id=?";
    ...
    private static void InsertNewHouse(House house)
    {
        try
        {
            ...
            stmt.Parameters.Add(new MySqlParameter { Value = DatabaseTimestamp.ToUnixTimeMillisecondsOrDbNull(house.GetAcquiredTime()) });
            stmt.Parameters.Add(new MySqlParameter { Value = house.GetPermissionsForDB() });
            stmt.Parameters.Add(new MySqlParameter { Value = DatabaseTimestamp.ToUnixTimeMillisecondsOrDbNull(house.GetNextPay()) });
    ...
    public static Dictionary<int, House> LoadHouses(ICollection<HousingLand> lands, bool studios)
    {
        ...
        try
        {
            ...
            while (rset.Read())
            {
                ...
                house.SetAcquiredTime(DatabaseTimestamp.ReadNullableDateTimeOffset(rset, "acquire_time_epoch_millis"));
                house.SetPermissionsFromDB(rset.GetInt32(rset.GetOrdinal("settings")));
                house.SetNextPay(DatabaseTimestamp.ReadNullableDateTimeOffset(rset, "next_pay_epoch_millis"));
