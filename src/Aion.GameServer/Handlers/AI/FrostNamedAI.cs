...
public class FrostNamedAI : PatternAi
{
    ...
    private static readonly AiPattern Pattern_ = new AiPattern
    {
        ...
        OnBattleTimer = Of(
            // --- 80-100 ------------------------------------------------------------------------------
            Branch(50, "single hit", [When.Timer(0), Healthy],
                Do.ArmTimer(1, 10000),
                Do.SkillOnTarget(EarthCleave)),
            AreaPair(49, on: 1, Healthy, delay: 14000),
            Branch(46, "single hit", [When.Timer(2), Healthy],
                Do.ArmTimer(3, 10500),
                Do.SkillOnTarget(EarthCleave)),
            Branch(45, "switch target, single hit", [When.Timer(3), Healthy],
                Do.ArmTimer(4, 10000),
                Do.SwitchTarget(AggroTarget.RANDOM),
                Do.SkillOnTarget(EarthCleave)),
            Branch(44, "centred and donut", [When.Timer(4), Healthy],
                Do.ArmTimer(0, 14000),
                Do.SkillOnSelf(TectonicShift),
                Do.SkillOnSelf(TectonicShift)),

            // --- 40-80 -------------------------------------------------------------------------------
            Branch(40, "random-target donut", [When.Timer(0), Middle],
                Do.ArmTimer(1, 11000),
                Do.SwitchTarget(AggroTarget.RANDOM),
                Do.SkillOnTarget(GelidImpel)),
            // Retail casts index 1 on itself here where every other band aims it at the target.
            Branch(39, "single hit", [When.Timer(1), Middle],
                Do.ArmTimer(2, 14000),
                Do.SkillOnSelf(EarthCleave)),
            AreaPair(38, on: 2, Middle, delay: 14000),
            Branch(35, "single hit", [When.Timer(3), Middle],
                Do.ArmTimer(4, 10000),
                Do.SkillOnTarget(EarthCleave)),
            Branch(34, "random-target donut, twice", [When.Timer(4), Middle],
                Do.ArmTimer(0, 14000),
