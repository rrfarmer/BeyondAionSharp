...
public class GameServerOptionsTests
{
	...
	public void LoadFromJavaConfig_ReadsCoreAndNetworkDefaults()
	{
		...
		Assert.Equal(900, options.PeriodicSave.PlayerGeneralSeconds);
		Assert.Equal(900, options.PeriodicSave.PlayerItemsSeconds);
		...
		// connect_address defaults to ${socket_address} = 0.0.0.0:7777 as the config value; SM_GS_AUTH resolves the
		// unspecified 0.0.0.0 to a reachable address (loopback) when advertising it to clients.
	...
	public void LoadFromJavaConfig_AppliesMyGsOverridesLast()
	{
		...
		try
		{
			...
			File.WriteAllText(
				...
				gameserver.periodicsave.player.general = 31
				gameserver.periodicsave.player.items = 33
			...
			Assert.Equal(31, options.PeriodicSave.PlayerGeneralSeconds);
			Assert.Equal(33, options.PeriodicSave.PlayerItemsSeconds);
