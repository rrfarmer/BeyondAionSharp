...
def spawned_in_our_data():
    ...
    # And the generated tables, which are data with a .cs extension. TiamatDyingRotationAI reads its
    # whole rotation out of TiamatRotation.cs -- beacons, cyclops cracks, gravity bombs, all of it -- so
    # the class source names none of those ids and the audit ranked six of them as owed. Every one was
    # already placed, with retail's own counts.
    #
    # Restricted to files carrying the <auto-generated> banner, not siblings in general. An earlier
    # attempt to credit any sibling file made 28 correctly-clean classes look like they disagreed; a
    # generated table is a narrower and checkable thing.
    for gen in (REPO / "src").rglob("*.cs"):
        if "/obj/" in gen.as_posix() or "/bin/" in gen.as_posix():
            continue
        head = gen.read_text(encoding="utf-8", errors="replace")[:400]
